...
<?php get_sidebar(); ?>
<div id="content">
  <h1 class="name"><a href="<?php bloginfo('url'); ?>" title="<?php bloginfo('name'); ?>"><?php bloginfo('name'); ?></a></h3>
  <p class="description"><?php bloginfo('description'); ?></p>
  <div class="line"></div>
  <div class="primary">
    <?php if ( is_day() ) : ?>
    <h3 class="category"><?php printf( __( 'Daily: %s' ), '<span>' . get_the_date() . '</span>' ); ?></h3>
    <?php elseif ( is_month() ) : ?>
    <h3 class="category"><?php printf( __( 'Monthly: %s' ), '<span>' . get_the_date( 'F Y' ) . '</span>' ); ?></h3>
    <?php elseif ( is_year() ) : ?>
    <h3 class="category"><?php printf( __( 'Yearly: %s' ), '<span>' . get_the_date( 'Y' ) . '</span>' ); ?></h3>
    <?php elseif ( is_tag() ) : ?>
    <h3 class="category"><?php printf( __( 'Tag: %s' ), '<span>' . single_tag_title( '', false ) . '</span>' ); ?></h3>
    <?php else : ?>
    <h3 class="category"><?php _e( 'Blog Archives' ); ?></h3>
    <?php endif; ?>
    <div class="line"></div>
